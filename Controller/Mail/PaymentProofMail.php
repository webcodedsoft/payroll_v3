...
class Controller_Mail_PaymentProofMail
{
    ...
    public function PaymentProofMail($earning_ids)
    {
        
        foreach ($earning_ids as $key => $single_manage_payroll_select_id) {

            $earning_result = $this->_query->query("SELECT * FROM earning WHERE ID = ? ORDER BY ID DESC LIMIT 1", array($single_manage_payroll_select_id))->first();
            ...
            $payment_proof_html_file = $this->mail_file->PaymentProofHTML($single_manage_payroll_select_id);
            ...
            $url_path = Classes_Session::get("Loggedin_Session")."/Payment_Proof/";

            if(Classes_Folder::create($url_path)){
                $url_path = "../../Folders/Company_Folders/" . Classes_Session::get("Loggedin_Session") . "/Payment_Proof/" . $first_name.'_'.date("Y_m_d"). ".pdf";
            } else {
                $url_path = "../../Folders/Company_Folders/" . Classes_Session::get("Loggedin_Session") . "/Payment_Proof/" . $first_name . '_' . date("Y_m_d") . ".pdf";
            }
    ...
    public function PaymentProofDownload($earning_id)
    {

        $earning_result = $this->_query->query("SELECT * FROM earning WHERE ID = ? ORDER BY ID DESC LIMIT 1", array($earning_id))->first();

        $employee_data_values = $this->data_manager->EmployeesDataByID($earning_result["Employee_ID"])->first();

        $first_name = $employee_data_values["First_Name"];

        $payment_proof_html_file = $this->mail_file->PaymentProofHTML($earning_id);
        $html_code = $this->__css;
        $html_code .= $payment_proof_html_file;
        $this->__dompdf->load_html($html_code);
        $this->__dompdf->setPaper('A4', 'portrait');
        ...
        $file = $this->__dompdf->output();
        if (file_put_contents($url_path, $file) || !file_put_contents($url_path, $file)) {
